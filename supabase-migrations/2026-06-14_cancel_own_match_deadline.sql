-- Item 5 Phase 4: persist caller-computed cancelled_after_deadline instead of hardcoded true.
-- Frontend supplies p_cancelled_after_deadline via canEditFlight (Item 7 servicePeriods).

begin;

drop function if exists public.cancel_own_match(bigint);

create or replace function public.cancel_own_match(
  p_ride_id bigint,
  p_cancelled_after_deadline boolean
)
returns jsonb
language plpgsql
security definer
...
revoke execute on function public.cancel_own_match(bigint, boolean) from public, anon;
grant execute on function public.cancel_own_match(bigint, boolean) to authenticated, service_role;
